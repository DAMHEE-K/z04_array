...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		// 1-1. 변수 선언
		int num;
		// Stack 영역에 int 자료형을 저장할 수 있는 공간 4byte를 생성(할당)
		...
		// 2-1. 배열 선언
		int[] arr;
		// Stack 영역에 int[] (int 배열) 자료형 공간을 4btye 할당하고
		// 그 공간에 arr이라는 이름을 부여
		...
		// 2-3. 배열 요소에 값 대입
		// arr은 int[] 참조형 변수이지만,
		// arr[0]는 int 자료형 변수이기 때문에 정수 값을 대입할 수 있다.
		...
		// 2-4. 배열 요소에 값 읽어오기
		System.out.println(arr[0]); // arr이 참조하고 있는 배열의 0번째 인덱스 값을 얻어온다
		System.out.println(arr[1]); // arr이 참조하고 있는 배열의 1번째 인덱스 값을 얻어온다
		System.out.println(arr[2]); // arr이 참조하고 있는 배열의 2번째 인덱스 값을 얻어온다
	...
	public void ex2() {
		...
		// 1) stack 영역에 int[] 자료형인 참조형 변수 arr 선언
	...
	// null : 참조형 변수가 선언되었으나 아무곳도 참조하고 있지 않음
	...
	public void ex4() {
		...
		// 입력 받은 인원수 만큼의 점수를 입력 받아 배열에 저장
		// 입력이 완료되면 점수의 합계, 평균, 최고점, 최저점 출력
		...
		int input = sc.nextInt(); // 인원수
		
		// 배열 선언 및 할당
		// 할당할 배열의 크기는 입력 받은 크기 만큼
		...
		int max = score[0];
		int min = score[0];
		// 아래에 for문을 이용해서 score 배열에 있는 모든 값과 max, min을 비교
		// 이 때,
	...
	public void ex5() {
		...
		System.out.println(Arrays.toString(arr)); // char 배열은 String
		...
		// 각각 'A', 'B', 'C', 'D', 'E'로 초기화 후 주소를 arr3에 대입한다
	...
	public void ex8() {
		...
		// 입력받은 값과 일치하는 값이 있으면서 인덱스 번호를 출력
		// 없으면 "존재하지 않음"
	...
	public void ex9() {
		...
		char ch = sc.nextLine().charAt(0);
				// String.charAt(0) : 문자열의 제일 앞 문자 얻어오기
		...
		} else { // 3. 단, 일치하는 문자가 없을 경우 "존재하지 않습니다." 출력 
